...
System.out.println("已经注册过");
        	...
        	try {
    			JdbcUtils.getQueryRuner().update(
    					"insert into admin(id,username,pwd,email,state) values(?,?,?,?,?)",UUID.randomUUID().toString(),
	...
	public String getUUID(Admin admin){
		String sql = "select * from admin where email=?";   //根据用户的email查找到数据库中存放的整个对象
		try {
			//执行该SQL语句并返回一个admin对象
			admin =  JdbcUtils.getQueryRuner().query(sql,
					new BeanHandler<Admin>(Admin.class), admin.getEmail() //根据email查找数据库中是否存在该用户
	...
	public void updateState(Admin admin){
		try {
			...
			if(admin != null){
				...
				//将结果写回数据库
				JdbcUtils.getQueryRuner().update(
    					"update admin set state=? where email=?",admin.isState(),admin.getEmail());
			}else {
				return;
			}
